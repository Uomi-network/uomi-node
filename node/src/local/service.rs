...
pub fn start_node(
    ...
    let service_version: pallet_uomi_engine::types::Version =
        ai_status["UOMI_ENGINE_PALLET_VERSION"].as_u64().unwrap()
            as pallet_uomi_engine::types::Version;
    ...
    if !service_cuda_available {
        log::error!(
            "🚨 AI service is not available to use CUDA. Please enable CUDA on the AI service."
        );
    } else {
